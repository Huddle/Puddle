        ...
        public class AccessDBProvider : NavigationCmdletProvider
        {
                ...
                else
                {
                    ...
                    if (type == PathType.Table)
                    {
                        ...
                        foreach (DatabaseRowInfo row in GetRows(tableName))
                        {
                            WriteItemObject(
                                row,
                                path + pathSeparator + row.RowNumber,
                                false);
            ...
            protected override void CopyItem(string path, string copyPath, bool recurse)
            {
                ...
                if (type == PathType.Table)
                {
                    ...
                    {
                        var e = new ArgumentException("Table can only be copied on to another table location");
            ...
            protected override void MoveItem(string path, string destination)
            {
                ...
                PathType destType = GetNamesFromPath(
                    destination,
                    out destTableName,
                    out destRowNumber);
                ...
                if (type == PathType.Table)
                {
                    var e = new ArgumentException("Move not supported for tables");
            ...
            private string[] ChunkPath(string path)
            {
                ...
                string pathNoDrive = normalPath.Replace(
                    PSDriveInfo.Root + pathSeparator,
                    string.Empty);
            ...
            {
                var message = new StringBuilder("Path must represent either a table or a row :");
                message.Append(path);
            ...
            {
                var exp = new Regex(pattern, RegexOptions.Compiled | RegexOptions.IgnoreCase);
            ...
            private bool TableIsPresent(string tableName)
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    var name = dr["TABLE_NAME"] as string;
